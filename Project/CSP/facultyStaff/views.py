...
from facultyStaff.models import FacultyStaff
...
def attendance(request, course_code):
    redirect = checkIdentity(request, 'facultystaff')
    if redirect == "":
        template = loader.get_template('facultystaff/_attendance.html')
        current = FacultyStaff.objects.filter(username=request.session['username'])[0]
...
def grades(request, course_code):
    ...
    if redirect == "":
            ...
            if request.method == 'POST':
                ...
                for name,marks,total in UpdateList:
                    try:
                        ...
                        l.save()      
        template = loader.get_template('facultystaff/_grades.html')
        current = FacultyStaff.objects.filter(username=request.session['username'])[0]
        course = Course.objects.filter(pk=course_code)[0]
        course_semester = course.semester_offered
        if course_semester[0] == "M":
        ...
        # ListofMarks1 = Marks.objects.filter(coursecode=course.id, category='MD1')
        # ListofMarks2 = Marks.objects.filter(coursecode=course.id, category='MD2')
        # ListofMarks3 = Marks.objects.filter(coursecode=course.id, category='ESM')
        # k = ListofMarks1.count()
        # l = ListofMarks2.count()
        # m = ListofMarks3.count()
...
def viewStudentProfile(request, roll_no):
    redirect = checkIdentity(request, 'facultystaff')
    if redirect == "":
        template = loader.get_template('facultystaff/_viewStudent.html')
        current = FacultyStaff.objects.filter(username=request.session['username'])[0]
        return studentViewStudentProfile(request, roll_no, 'facultystaff/_viewStudent.html', current)
    return HttpResponseRedirect(redirect)
...
def reports(request):
    ...
    if redirect == "":
        current = FacultyStaff.objects.filter(username=request.session['username'])[0]
        return Reports(request, current, 'facultystaff/_reports.html', 'facultystaff/_results.html','facultystaff')
    return HttpResponseRedirect(redirect)
...
def emailOrExport(request):
    ...
    return HttpResponseRedirect('/facultystaff/reports/')
